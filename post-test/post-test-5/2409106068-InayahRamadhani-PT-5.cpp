...
void tampilkanKarakter(string *nama, Karakter *karakter, int jumlahKarakter);
    ...
    if (pilihan == "1") DaftarKarakter(karakter, jumlah);
        ...
        for (int i = 0; i < jumlah; i++) {
            if ((karakter + i)->nama == namaCari) {
                string *namaPtr = &((karakter + i)->nama);
                tampilkanKarakter(namaPtr, karakter, jumlah);
                ditemukan = 1;
                break;
            }
        }
...
// menghitung jumlah karakter yang digunakan saat login
int hitungTotalKarakter(Tim *tim, int idx) {
    if (idx < 0) return 0;
    int jumlah = 0;
